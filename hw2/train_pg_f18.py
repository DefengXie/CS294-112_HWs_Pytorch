...
class policeNN(nn.Module):
    def __init__(self,input_dim, output_dim, n_layers,
        ...
        for i in range(1,n_layers):
            self.layers.append(nn.Sequential(nn.Linear(self.input_dim,self.hide_dim),nn.ReLU() if activation == 'relu' else nn.Tanh()))
...
def build_mlp(input_placeholder, output_size, scope, n_layers, hide_size,
...
class Agent(object):
    ...
    #                           ----------PROBLEM 2----------
    #========================================================================================#
    #def define_placeholders(self,x,y,z):
    #    """
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            sy_mean = self.policy(sy_ob_no)
            sy_logstd = nn.Parameter(torch.randn(self.ac_dim,requires_grad=True))
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            sy_sampled_ac = torch.squeeze(torch.multinomial(sy_logits_na,1),dim=1)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            sy_sampled_ac = sy_mean + torch.exp(sy_logstd) * torch.randn_like(sy_mean)
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            sy_logprob_n = nn.functinal.cross_entropy(nn.Softmax(sy_logits_na),sy_ac_na)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
    ...
    def build_computation_graph(self,sy_ob_no, sy_ac_na, sy_adv_n,sy_target_n):
        """
            Notes on notation:
        ...
        self.policy_parameters = self.policy_forward_pass(sy_ob_no)
        ...
        self.sy_logprob_n = self.get_log_prob(self.policy_parameters,sy_ac_na)

        #========================================================================================#
        #                           ----------PROBLEM 2----------
        # Loss Function and Training Operation
        #========================================================================================#
        loss = tf.reduce_mean(self.sy_logprob_n*sy_adv_n)  # YOUR CODE HERE
        loss.backpack()
        ...
        #========================================================================================#
        if self.nn_baseline:
            self.baseline_prediction = self.policy_baseline(sy_ob_no)
            # YOUR_CODE_HERE
            #self.sy_target_n = None
            baseline_loss = torch.nn.functional.mse_loss(self.baseline_prediction,sy_target_n)
            baseline_loss.backpack()
            self.policy_baseline.step()
...
def main():
    ...
    parser.add_argument('--pg_step', '-ps', type=int, default=3)
